...
uint32_t USART3_RX_STA = 0;
char *USART3_RX_BUF = nullptr;
...
static u8 *JudgeStr(u16 waittime) {
  ...
  while (--waittime) {
    ...
    {
      USART3_RX_STA = 0;
      data = strstr(USART3_RX_BUF, (const char *)str);
...
// 储存模板 PS_StoreChar
// 功能:将 CharBuffer1 或 CharBuffer2 中的模板文件存到 PageID
// 号flash数据库位置。 参数:  BufferID @ref charBuffer1:0x01	charBuffer1:0x02
//        PageID（指纹库位置号）
// 说明:  模块返回确认字
...
// BufferID，
// StartPage(起始页)，PageNum（页数） 说明:  模块返回确认字+页码（相配指纹模板）
